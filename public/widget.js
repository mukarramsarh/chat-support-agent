...
(function () {
  ...
  function render() {
      ...
      fetch(API_BASE + '/api/chat/message', {
        ...
        function handleEvent(raw) {
          ...
          if (event === 'meta' && payload.conversation_id) {
            ...
            // Session-limit handoff asks us to lock the composer (message text
            // arrives as normal tokens above).
            if (payload.lock) { lockInput(); }
          } else if (event === 'error') {
    ...
    // Session-limit handoff disables the composer so the visitor is routed to
    // contact the team directly (the message + details arrive as normal tokens).
    function lockInput() {
      input.disabled = true;
